...
@login_required
def new_conversation_with_user(request, user_id):
    # Sprawdź, czy istnieje już rozmowa między obecnym użytkownikiem a wybranym użytkownikiem
    ...
    # Jeśli rozmowa już istnieje, przekieruj do jej szczegółów
    if conversations.exists():
        return redirect('conversation:detail', pk=conversations.first().id)

    # Jeśli rozmowa nie istnieje, utwórz nową
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            conversation.members.add(request.user)
            conversation.members.add(other_user)
            conversation.save()
